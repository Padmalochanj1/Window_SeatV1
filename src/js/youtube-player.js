...
const RAINY_TRAIN_YOUTUBE = {
  musicPlaylistId: 'PLEyzK5ZPrgtI',
  musicPlaylistUrl: 'https://music.youtube.com/playlist?list=PLEyzK5ZPrgtI',
...
function onYouTubeIframeAPIReady() {
  ...
  if (RAINY_TRAIN_YOUTUBE.trainVideoId) {
...
function startTrainSound() {
  if (!RAINY_TRAIN_YOUTUBE.trainVideoId || !trainYT) return;
  if (!trainReady) {
...
function setMusicLoop(enabled) {
  if (musicReady && musicYT && typeof musicYT.setLoop === 'function') {
    musicYT.setLoop(Boolean(enabled));
  }
}
...
function rainyTrainToggleMusic() {
  ...
  if (state === YT.PlayerState.PLAYING) musicYT.pauseVideo();
  else startMusicAndTrain();
}
